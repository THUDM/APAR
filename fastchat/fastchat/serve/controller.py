...
class Controller:
    ...
    def get_worker_count(self, model_name: str):
        worker_counter = 0
    ...
    def worker_trace(self, params):
        worker_addr = self.get_worker_address(params["model"])
        print("dispatching to worker: ", worker_addr)
        if not worker_addr:
            return self.handle_no_worker(params)
        response = requests.post(
            worker_addr + "/worker_trace",
            json=params,
            stream=True,
            timeout=WORKER_API_TIMEOUT,
        )
...
@app.post("/get_worker_count")
async def get_worker_address(request: Request):
    data = await request.json()
    count = controller.get_worker_count(data["model"])
    return {"count": count}
...
async def worker_trace(request: Request):
    params = await request.json()
    trace_dict = controller.worker_trace(params)
    return JSONResponse(trace_dict)
