...
class ModelWorker(BaseModelWorker):
    ...
    def generate_stream_gate(self, params):
        ...
        if self.apar:
            import importlib
            import fastchat.serve.apar_stream as apar_stream
            importlib.reload(apar_stream)
            self.generate_stream_func = apar_stream.stream_chat_tree

        print("generate_stream_gate with", self.generate_stream_func)
    ...
    def trace_gate(self, params):
        assert self.apar, "trace_gate is only for apar"
        import importlib
        import fastchat.serve.apar_stream as apar_stream
        importlib.reload(apar_stream)

        outputs = apar_stream.get_trace(
            self.model,
            self.tokenizer,
            params,
        )
...
async def api_generate_stream(request: Request):
    ...
    print("Get req worker_generate_stream", params)
...
async def api_generate(request: Request):
    params = await request.json()
    print("Get req worker_generate", params)
    await acquire_worker_semaphore()
    output = worker.generate_gate(params)
    release_worker_semaphore()
    return JSONResponse(output)

@app.post("/worker_trace")
async def api_generate(request: Request):
    params = await request.json()
    print("Get req worker_trace", params)
    await acquire_worker_semaphore()
    output = worker.trace_gate(params)
    release_worker_semaphore()
...
def create_model_worker():
    ...
    parser.add_argument("--apar", default=False, action="store_true")
