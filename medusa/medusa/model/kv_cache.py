...
class KVCache:
    ...
    def cat(self, tensor: torch.Tensor, dim: int = 2):
        ...
        dst[:tensor.shape[0]].copy_(tensor)  # only update current available batches
        self.current_length.add_(tensor.shape[dim])
        return torch.narrow(self.data, 2, 0, self.current_length)[:tensor.shape[0]]


def initialize_past_key_values(model, batch_size: int = 1, content_length: int = 2048):
    """
