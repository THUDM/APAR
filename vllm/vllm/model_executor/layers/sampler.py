...
class Sampler(nn.Module):
    ...
    def forward(
        ...
        if logits.shape[-1] > 32000:  # APAR sequence
            logits[:, 32001] = logits.min()  # ban split_sop
            # for seq_id, sdata in input_metadata.seq_data.items():
            #     if sdata.output_token_ids and (32001 in sdata.output_token_ids or sdata.output_token_ids[-1] == 32000 or sdata.output_token_ids.count(32000) > -1): # no split_mask after split_sop, no consecutive split_mask
            #         logits[input_metadata.seq_input_pos[seq_id], 32000] = logits.min()  # ban split_mask
